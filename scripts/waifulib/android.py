...
def options(opt):
	...
	grp.add_option('--javac-source-target', dest='JAVAC_SOURCE_TARGET', default="1.6", action="store",
		help="set java compiler source and target [default: %default]")

	grp.add_option( "--key", dest="JAVA_KEYSTORE", default=None, action="store",
		help="set keystore with alias and passwords (ex. --key=mykey.pfx,testalias,keypass,pass)" )

	grp.add_option('--termux', dest='termux', default=False, action='store_true',
		help='configure to use tools included in termux [default: %default]')
	return
...
def configure(conf):
	...
	conf.env.JAVAC_SOURCE_TARGET = conf.options.JAVAC_SOURCE_TARGET
	conf.msg('Javac -source and -target', conf.env.JAVAC_SOURCE_TARGET)

	if conf.options.JAVA_KEYSTORE:
		ks = conf.options.JAVA_KEYSTORE.split( "," )
		if len( ks ) != 4:
			conf.fatal( "Invalid --key!" )

		conf.env.JKEYSTORE = ks[0]
		conf.env.JKEYALIAS = ks[1]
		conf.env.JKEYPASSW = ks[2]
		conf.env.JKEYPASS2 = ks[3]
	else:
		conf.env.JKEYSTORE = "debug.keystore"
		conf.env.JKEYALIAS = "androiddebugkey"
		conf.env.JKEYPASSW = "android"
		conf.env.JKEYPASS2 = "android"
...
def apply_d8(self):
	...
	try:
		...
		self.env.KS_ALIAS = self.env.JKEYALIAS
		self.env.KS_PASS  = "pass:" + self.env.JKEYPASS2
		self.env.KEY_PASS = "pass:" + self.env.JKEYPASSW
